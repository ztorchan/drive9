...
#   SMALL_OPS              legacy alias for FIO_SMALL_TOTAL_FILES (default: 200000)
#   FIO_BIN                existing fio executable (required; no network auto-fetch)
#   FIO_RUNS               fio repetitions per workload (default: 5)
#   FIO_SMALL_TOTAL_FILES  total files per small-file workload case, split across workers (default: 200000)
...
#   MDTEST_TOTAL_ENTRIES  total files/dirs per mdtest case, split across MPI ranks (default: 200000)
...
#   FIO_ALLOC_SIZE_KIB     fio smalloc pool size per pool in KiB (default: auto; scales for >=50000 files)
#   FIO_OPEN_FILES         files kept open per fio job (default: 1)
...
#   KIMI_SMALL_OPS         total operations per small-file case (default: 200000)
...
FIO_ALLOC_SIZE_KIB="${FIO_ALLOC_SIZE_KIB:-}"
FIO_OPEN_FILES="${FIO_OPEN_FILES:-1}"
...
MDTEST_TOTAL_ENTRIES="${MDTEST_TOTAL_ENTRIES:-${MDTEST_FILES_PER_RANK:-200000}}"
...
KIMI_SMALL_OPS="${KIMI_SMALL_OPS:-200000}"
...
if [[ -n "$FIO_ALLOC_SIZE_KIB" ]]; then
  require_positive_int FIO_ALLOC_SIZE_KIB "$FIO_ALLOC_SIZE_KIB"
fi
require_positive_int FIO_OPEN_FILES "$FIO_OPEN_FILES"
...
fio_alloc_size_kib_for_total() {
  local total_files=$1
  local alloc_size_kib

  [[ "$total_files" =~ ^[0-9]+$ && "$total_files" -gt 0 ]] || \
    fail "fio total file count must be a positive integer: ${total_files}"
  if [[ -n "$FIO_ALLOC_SIZE_KIB" ]]; then
    printf '%s' "$FIO_ALLOC_SIZE_KIB"
    return 0
  fi
  if (( total_files < 50000 )); then
    printf '0'
    return 0
  fi

  # fio 3.36 allocates file metadata from eight 16 MiB pools by default.
  # Budget 4 KiB per file across those eight pools and round the per-pool
  # value to 4 MiB. This gives about 4 GiB of smalloc space for one million
  # files, and also covers the 50,000-file case that exhausted the default
  # pools. The value remains overrideable per host.
  alloc_size_kib=$(((total_files + 1) / 2))
  alloc_size_kib=$(((alloc_size_kib + 4095) / 4096 * 4096))
  if (( alloc_size_kib < 131072 )); then
    alloc_size_kib=131072
  fi
  printf '%s' "$alloc_size_kib"
}
...
summarize_fio_result() {
  ...
  local run_index=${11}
  local alloc_size_kib=${12}
  local open_files=${13}

  python3 - "$raw_file" "$summary_file" "$scenario" "$case_id" "$phase" "$workload" \
    "$file_size_bytes" "$num_jobs" "$files_per_job" "$total_files" "$run_index" "$alloc_size_kib" "$open_files" <<'PY'
import json
import pathlib
import sys
...
alloc_size_kib = int(sys.argv[12])
open_files = int(sys.argv[13])
...
run_fio_workload() {
  ...
  local alloc_size_kib="$FIO_ALLOC_SIZE_KIB"
  local fio_rc=0
  ...
  alloc_size_kib="$(fio_alloc_size_kib_for_total "$total_files")"
  ...
  if (( alloc_size_kib > 0 )); then
    args+=("--alloc-size=${alloc_size_kib}")
  fi
  args+=(
    "--name=${fio_job_name}"
    "--directory=${data_dir}"
  ...
  effective_timeout="$(case_timeout_remaining "$timeout_seconds")" || return $?
  log "fio: scenario=${scenario} phase=${phase} workload=${workload} size=${file_size_bytes}B jobs=${num_jobs} total_files=${total_files} files_per_job=${files_per_job} actual_files=$((num_jobs * files_per_job)) alloc_size_kib=${alloc_size_kib} run=${run_index} timeout=${effective_timeout}s"
  ...
  summarize_fio_result "$raw_file" "$summary_file" "$scenario" "$case_id" "$phase" "$workload" \
    "$file_size_bytes" "$num_jobs" "$files_per_job" "$total_files" "$run_index" "$alloc_size_kib" "$FIO_OPEN_FILES" >/dev/null || return $?
}
...
summarize_fio_matrix() {
  ...
  local num_jobs=$6
  local total_files=$7
  local alloc_size_kib=$8
  local open_files=$9
  local include_phase=${10:-}

  python3 - "$result_root" "$summary_file" "$scenario" "$case_id" "$file_size_bytes" "$num_jobs" "$total_files" "$alloc_size_kib" "$open_files" "$include_phase" <<'PY'
import json
import math
import pathlib
import statistics
...
alloc_size_kib = int(sys.argv[8])
open_files = int(sys.argv[9])
include_phase = sys.argv[10]

records = []
for path in sorted(result_root.rglob("*.summary.json")):
    data = json.loads(path.read_text(encoding="utf-8"))
    if include_phase and data.get("phase") != include_phase:
        continue
...
fio_root_for_mount() {
  ...
  local mount_point=$3
  if [[ "$scenario" == git ]]; then
    printf '%s' "${mount_point}/repo/agent-bench/fio/git/${size_kind}"
  else
    printf '%s' "${mount_point}/fio/${scenario}/${size_kind}"
  fi
}

run_fio_workload_runs() {
  local scenario=$1
  local case_id=$2
  local phase=$3
  local workload=$4
  ...
  for run_index in $(seq 1 "$FIO_RUNS"); do
    run_fio_workload "$scenario" "$case_id" "$phase" "$workload" "$fio_root" \
      "$file_size_bytes" "$num_jobs" "$total_files" "$run_index" "$result_root" "$timeout_seconds" || return $?
  done
}

run_fio_workload_case_body() {
  local scenario=$1 profile=$2 size_kind=$3 size_label=$4 file_size_bytes=$5
  local num_jobs=$6 total_files=$7 workload=$8 case_id=$9
  local remote_root="${10}" mount_point="${11}" cache_write="${12}" cache_read="${13}"
  local local_write="${14}" local_read="${15}" layer_name="${16}" result_root="${17}"
  local fio_root seed_workload measured_phase=write run_index timeout_seconds alloc_size_kib
  if [[ "$size_kind" == small ]]; then
    timeout_seconds="$SMALL_CASE_TIMEOUT_S"
  else
    timeout_seconds="$LARGE_CASE_TIMEOUT_S"
  fi
  ...
  if [[ "$scenario" == git ]] && ! prepare_git_workspace; then
    return 1
  fi
  fio_root="$(fio_root_for_mount "$scenario" "$size_kind" "$mount_point")"

  case "$workload" in
    seq_read)
      seed_workload=seq_write
      measured_phase=read
      ;;
    rand_read)
      seed_workload=rand_write
      measured_phase=read
      ;;
    *)
      seed_workload=""
  ...
  # Read workloads get their own tenant too. Seed the files in this tenant,
  # then remount with a fresh cache/local root before measuring the read path.
  if [[ -n "$seed_workload" ]]; then
    run_fio_workload "$scenario" "$case_id" prepare "$seed_workload" "$fio_root" \
      "$file_size_bytes" "$num_jobs" "$total_files" 1 "$result_root" "$timeout_seconds" || return $?
    if ! stop_mount; then
      return 1
    fi
  ...
  run_fio_workload_runs "$scenario" "$case_id" "$measured_phase" "$workload" "$fio_root" \
    "$file_size_bytes" "$num_jobs" "$total_files" "$result_root" "$timeout_seconds" || return $?
  alloc_size_kib="$(fio_alloc_size_kib_for_total "$total_files")"
  summarize_fio_matrix "$result_root" "${result_root}/matrix-summary.json" "$scenario" "$case_id" \
    "$file_size_bytes" "$num_jobs" "$total_files" "$alloc_size_kib" "$FIO_OPEN_FILES" "$measured_phase"
}

run_fio_workload_case() {
  local scenario=$1 profile=$2 size_kind=$3 size_label=$4 result_label=$5 file_size_bytes=$6
  local num_jobs=$7 total_files=$8 workload=$9 timeout_seconds=${10}
  local stamp case_id remote_root case_dir mount_point cache_write cache_read local_write local_read layer_name result_root
  stamp="$(date -u +%Y%m%dT%H%M%S)-$$"
  case_id="${scenario}-${size_kind}-${size_label}-c${num_jobs}-${workload}-${stamp}"
  remote_root="/bench-${case_id}/"
  case_dir="${WORK_DIR}/${case_id}"
  mount_point="${case_dir}/mount"
  ...
  layer_name="${case_id}"
  result_root="${RESULT_DIR}/fio/${scenario}/${size_kind}/${case_id}/${result_label}-c${num_jobs}"

  log "fio workload uses isolated tenant: scenario=${scenario} size=${size_label} jobs=${num_jobs} workload=${workload}"
  run_case_with_cleanup "$case_dir" "$timeout_seconds" run_fio_workload_case_body \
    "$scenario" "$profile" "$size_kind" "$size_label" "$file_size_bytes" "$num_jobs" \
    "$total_files" "$workload" "$case_id" "$remote_root" "$mount_point" "$cache_write" \
    "$cache_read" "$local_write" "$local_read" "$layer_name" "$result_root"
}
...
run_small_case() {
  ...
  local concurrency=$4
  local workload
  for workload in seq_write rand_write rand_rw fsync_write fdatasync_write seq_read rand_read; do
    run_fio_workload_case "$scenario" "$profile" small "${size}b" "${size}b" "$size" \
      "$concurrency" "$FIO_SMALL_TOTAL_FILES" "$workload" "$SMALL_CASE_TIMEOUT_S" || return $?
  done
}
...
run_large_case() {
  ...
  local num_jobs=$4
  local workload
  local file_size_bytes=$((large_mib * 1024 * 1024))
  local total_files=$((num_jobs * FIO_LARGE_FILES_PER_JOB))
  for workload in seq_write rand_write rand_rw fsync_write fdatasync_write seq_read rand_read; do
    run_fio_workload_case "$scenario" "$profile" large "${large_mib}m" "${large_mib}MiB" \
      "$file_size_bytes" "$num_jobs" "$total_files" "$workload" "$LARGE_CASE_TIMEOUT_S" || return $?
  done
}
...
main() {
  ...
  log "fio total files per small workload tenant: ${FIO_SMALL_TOTAL_FILES}"
  log "fio alloc size KiB: ${FIO_ALLOC_SIZE_KIB:-auto}"
  log "fio open files per job: ${FIO_OPEN_FILES}"
  log "fio runs per workload: ${FIO_RUNS}"
  log 'fio tenant isolation: one tenant per workload; read workloads seed and remount within that tenant'
